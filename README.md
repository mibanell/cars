# 🚗💰 How much is my car worth?
## 🤖 Prototype of a price prediction solution for second hand cars

<div align="center">
  <img src="img/cars.gif" style="width:100%;">
</div>

The purpose of this project was to create a complete machine learning prototype going from the exploratory data analysis to the model deployment and user interface development.

In this repository you can find:
1. [Exploratory Data Analysis](eda.ipynb)
2. [Neural network development with TensorFlow](train_model.ipynb)
3. [REST API model deployment with Flask](app.py)
4. [React frontend](react_frontend_app)

## Author

<br>
<p align="center">
  <img  src="img/signature.png" width="100">
</p>
<p align="center">
    <a href="https://github.com/mibanell">Miguel Ángel Ballester Granell</a>
    <br>
    <a href="https://www.linkedin.com/in/miguelangelballestergranell/">LinkedIn</a>
</p>